#!/usr/bin/env bash
# Stage-0 (4xH100 data-parallel): self-distill training data with the
# bidirectional DreamID-V-Faster teacher, split across 4 GPUs.
#
# Each GPU runs one process over a round-robin 1/4 slice of corpus.jsonl and
...
NUM_GPUS=${NUM_GPUS:-4}
GPU_LIST=${GPU_LIST:-0,1,2,3}             # physical GPUs to use, comma-separated
...
if [ "${#GPUS[@]}" -ne "${NUM_GPUS}" ]; then
  echo "[stage0-4gpu] ERROR: GPU_LIST has ${#GPUS[@]} entries but NUM_GPUS=${NUM_GPUS}" >&2
  exit 1
fi

# Auto-build the manifest from the paired input dir if not already present.
if [ ! -f "${MANIFEST}" ]; then
  echo "[stage0-4gpu] manifest not found -> building from ${INPUT_DIR}"
...
for f in yolox_l.onnx dw-ll_ucoco_384.onnx; do
  if [ ! -s "${POSE_DIR}/${f}" ] && [ -s "${MODELS_DIR}/${f}" ]; then
    ...
    echo "[stage0-4gpu] linked ${f} -> ${POSE_DIR}/${f}"
  fi
done

SHARD_DIRS=()
PIDS=()
echo "[stage0-4gpu] launching ${NUM_GPUS} workers over GPUs ${GPU_LIST}"
for i in $(seq 0 $((NUM_GPUS - 1))); do
  gpu=${GPUS[$i]}
  shard_dir="${OUTPUT_LMDB}_shard${i}"
  SHARD_DIRS+=("${shard_dir}")
  log="${LOGDIR}/stage0_shard${i}.log"
  echo "[stage0-4gpu]  shard ${i} -> GPU ${gpu}, out=${shard_dir}, log=${log}"
...
for i in $(seq 0 $((NUM_GPUS - 1))); do
  if wait "${PIDS[$i]}"; then
    echo "[stage0-4gpu] shard ${i} OK  ($(grep -h 'accepted' "${LOGDIR}/stage0_shard${i}.log" | tail -n1))"
  else
    echo "[stage0-4gpu] shard ${i} FAILED (see ${LOGDIR}/stage0_shard${i}.log):" >&2
    tail -n 15 "${LOGDIR}/stage0_shard${i}.log" >&2 || true
    fail=1
  fi
done
if [ "${fail}" -ne 0 ]; then
  echo "[stage0-4gpu] one or more shards failed; NOT merging. Fix and rerun." >&2
  exit 1
fi
...
echo "[stage0-4gpu] DONE -> ${OUTPUT_LMDB}"
