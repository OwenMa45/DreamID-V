...
# teacher inference.
echo "[stage0-8gpu] verifying merged LMDB ..."
...
print('[verify] entries=%d clean_latent_shape=%s' % (n, s)); sys.exit(0 if s else 1)"; then
  echo "[stage0-8gpu] merged LMDB NOT readable at ${OUTPUT_LMDB}; KEEPING shards for re-merge." >&2
  echo "[stage0-8gpu] re-merge with: python -m tools.merge_lmdb_shards --shards ${SHARD_DIRS[*]} --output ${OUTPUT_LMDB}" >&2
  exit 1
fi
