...
# Generator = Stage-3 (8-GPU run) step-500 DMD ckpt (1000-step run, ckpt every 100):
# checkpoints/chunkwise/stage3_dmd_8h200/checkpoint_model_000500/model.pt
# (already baked into configs/inference_8h200.yaml, no manual edit needed).
set -euo pipefail
...
# Inference is single-GPU; pick one of the 8 cards.
...
SAVE_FILE=${SAVE_FILE:-${PROJECT_ROOT}/outputs/swapped_8h200.mp4}
...
python inference.py \
  --config_path configs/inference_8h200.yaml \
