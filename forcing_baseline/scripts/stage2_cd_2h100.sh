#!/usr/bin/env bash
# Stage-2 (2xH100): causal Consistency Distillation (CD).
...
# (already baked into configs/causal_cd_2h100.yaml, no manual edit needed).
set -euo pipefail
...
NPROC=${NPROC:-2}
LOGDIR=${LOGDIR:-checkpoints/chunkwise/stage2_cd_2h100}
mkdir -p "${LOGDIR}"
